...
create policy "users read own listing reports" on public.listing_reports for select using (auth.uid() = reporter_id);
drop policy if exists "users create listing reports" on public.listing_reports;
create policy "users create listing reports" on public.listing_reports for insert to authenticated with check (auth.uid() = reporter_id and exists (select 1 from public.listings l where l.id = listing_id and l.seller_id <> auth.uid()));
drop policy if exists "users update own open listing reports" on public.listing_reports;
create policy "users update own open listing reports" on public.listing_reports for update using (auth.uid() = reporter_id and status = 'open') with check (auth.uid() = reporter_id);
