-- Fix catalog ingest for source keys such as catalog:kautek.
-- ai_catalog_records.source_id is a UUID FK, while the catalog workers use
-- human-readable source keys. Keep those keys in source_ids and only populate
-- source_id when the incoming value is an actual UUID.
