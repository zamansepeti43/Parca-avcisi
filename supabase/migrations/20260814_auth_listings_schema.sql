-- Parça Avcısı MVP — listings (marketplace) schema migration.
-- Applies the base marketplace schema (schema.sql) to the Supabase database
-- as an idempotent migration: existing tables are preserved, nothing is
-- duplicated, and RLS policies are recreated safely.
-- Safe to run on a fresh database or on one where schema.sql was already applied.
...
-- ============================= RLS Policies =============================
-- Policies are dropped/recreated so this migration can be safely rerun.
