...
HEADERS = {'User-Agent': 'Parca-Avcisi-Catalog-Worker/2.3'}
...
UPLOAD_STATS = {'uploaded': 0, 'rejected': 0}
...
SOURCES = {
    ...
    'phira': {'brand': 'PHIRA', 'url': 'https://phira.com.tr/marka-kataloglarimiz/', 'quality': 0.98, 'kind': 'html'},
...
VEHICLE_MAKES = {
    'MERCEDES', 'MERCEDES-BENZ', 'MAN', 'VOLVO', 'SCANIA', 'DAF', 'RENAULT', 'IVECO', 'FORD',
    'BMC', 'ISUZU', 'MITSUBISHI', 'OTOKAR', 'FIAT', 'AUDI', 'BMW', 'VOLKSWAGEN', 'VW', 'OPEL',
    'PEUGEOT', 'CITROEN', 'TOYOTA', 'NISSAN', 'HYUNDAI', 'KIA', 'HONDA', 'SKODA', 'ŠKODA', 'SEAT',
    'DACIA', 'CHERY', 'MG', 'TOGG', 'KARSAN', 'TEMSA', 'FUSO', 'SUZUKI', 'MAZDA', 'JEEP', 'LAND ROVER',
    ...
    for attempt in range(1, 4):
        try:
            ...
            if response.status_code >= 500 and attempt < 3:
                ...
                continue
            return False, response.text[:2000]
...
def upload(rows):
    ...
    def send(chunk, label):
        if not chunk:
            return
        ok, body = post_batch(chunk, headers)
        if ok:
            UPLOAD_STATS['uploaded'] += len(chunk)
            print(f'uploaded {label}: {len(chunk)}', flush=True)
            return
        status = getattr(body, 'status_code', None)
        detail = body if isinstance(body, str) else json.dumps(body, ensure_ascii=False)
        print(f'Supabase upload failed {label}: status={status} detail={detail[:2000]}', flush=True)
        if len(chunk) == 1:
            UPLOAD_STATS['rejected'] += 1
            row = chunk[0]
            print(f"REJECTED record: brand={row.get('brand')} part={row.get('partNumber')} url={row.get('sourceUrl')}", flush=True)
            return
        mid = max(1, len(chunk) // 2)
        send(chunk[:mid], label + 'a')
        send(chunk[mid:], label + 'b')

    for i in range(0, len(rows), BATCH):
        send(rows[i:i + BATCH], str(i // BATCH + 1))
...
def discover_html_urls(source, base):
    urls = discover_sitemap(base)
    base_host = urlparse(base).netloc
    selected = []
    for url in urls:
        lower = url.lower()
        if any(k in lower for k in ('product', 'urun', 'catalog', 'katalog', 'item', 'parca', 'part', 'p/')):
            selected.append(url)

    # Some Turkish catalog sites do not expose a sitemap. In that case crawl the
    # catalog landing page and collect same-site product/model/category links.
    if not selected:
        try:
            html = get(base).text
            soup = BeautifulSoup(html, 'html.parser')
            for anchor in soup.find_all('a', href=True):
                ...
                elif any(k in lower for k in ('product', 'urun', 'catalog', 'katalog', 'item', 'parca', 'part')):
                    selected.append(href)
        except Exception as exc:
            print(f'landing-page discovery skip {base}: {exc}', flush=True)

    if not selected:
        selected = [base]
    return list(dict.fromkeys(selected))[:MAX_URLS]
...
def run_html(source):
    cfg = SOURCES[source]
    product_urls = discover_html_urls(source, cfg['url'])
...
def run_pdf_page(source):
    cfg = SOURCES[source]
    soup = BeautifulSoup(get(cfg['url']).text, 'html.parser')
    links = []
    for anchor in soup.find_all('a', href=True):
        href = urljoin(cfg['url'], anchor['href'])
        label = clean(anchor.get_text(' ', strip=True)).lower()
        if href.lower().endswith('.pdf') and (source != 'continental' or any(k in label for k in ('catalog', 'workbook', 'cross', 'products', 'catalogue'))):
            links.append(href)
    ...
    for index, url in enumerate(links, 1):
        ...
        try:
            with tempfile.NamedTemporaryFile(suffix='.pdf', delete=False) as file:
                path = file.name
            try:
                Path(path).write_bytes(get(url, timeout=max(HTTP_TIMEOUT, 60)).content)
            except Exception as exc:
                failed += 1
                print(f'{source}: pdf {index}/{len(links)} SKIP download | {url} | {exc}', flush=True)
                continue
            try:
                output = subprocess.run(['pdftotext', '-layout', path, '-'], capture_output=True, text=True, timeout=180)
            except Exception as exc:
                failed += 1
            ...
            if output.returncode != 0:
                failed += 1
                print(f'{source}: pdf {index}/{len(links)} SKIP parse | {url} | {output.stderr.strip()}', flush=True)
                continue
            ...
            for line in (clean(x) for x in output.stdout.splitlines()):
                ...
                for part in parts[:8]:
                    ...
                    row = record(source, part, line[:180], oems=OEM_RE.findall(line)[:10], apps=apps, source_url=url)
                    if row:
...
def main():
    ...
    if UPLOAD_STATS['uploaded'] == 0:
        raise RuntimeError(f'{source}: normalized {unique} records but Supabase accepted none; rejected={UPLOAD_STATS["rejected"]}')
    print(f'{source}: SUCCESS | unique={unique} | uploaded={UPLOAD_STATS["uploaded"]} | rejected={UPLOAD_STATS["rejected"]}', flush=True)
