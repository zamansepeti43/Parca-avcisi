...
const TIMEOUT = Number(process.env.CATALOG_HTTP_TIMEOUT || 20);
const HEADERS = {
...
async function get(url) {
  try {
    const r = await fetch(url, { headers: HEADERS, redirect: 'follow', signal: AbortSignal.timeout(TIMEOUT * 1000) });
    if (r.ok) return await r.text();
    throw new Error(`HTTP ${r.status}`);
  } catch (first) {
    const args = ['-fsSL', '--retry', '2', '--retry-delay', '1', '--connect-timeout', String(TIMEOUT), '--max-time', String(TIMEOUT * 2), '-A', HEADERS['user-agent'], '-H', `Accept: ${HEADERS.accept}`, url];
    try {
      const { stdout } = await execFileAsync('curl', args, { maxBuffer: 20 * 1024 * 1024 });
      if (!stdout) throw new Error('empty response');
      console.log(`phira: curl fallback OK ${url}`);
      return stdout;
    } catch (second) {
      throw new Error(`HTTP fetch failed: ${first?.message || first}; curl: ${second?.message || second}`);
    }
  }
}
...
function linksFrom(html) {
  ...
  while ((m = re.exec(html))) {
    ...
    if (parsed.hostname !== 'phira.com.tr') continue;
    const p = parsed.pathname.replace(/\/+$/, '') + '/';
    if (p.startsWith('/katalog/') && p !== '/katalog/') out.add(`${BASE}${p}`);
  }
  return [...out];
}
...
async function main() {
  ...
  let urls = [...new Set(linksFrom(landing))];
  if (!urls.length) urls = FALLBACK_SLUGS.map(slug => `${BASE}/katalog/${slug}/`);
      ...
      for (const line of lines) {
        for (const row of makeRows(line, url)) {
          ...
          if (pending.length >= BATCH) {
            ...
            console.log(`uploaded ${pending.length}; unique=${seen.size}`);
            pending = [];
          }
        }
      }
