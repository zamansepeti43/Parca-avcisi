#!/usr/bin/env python3
"""Extract an official MANN catalog PDF and push verified records to Supabase."""
import hashlib
import json
import os
import re
...
def download_pdf() -> None:
    ...
    req = urllib.request.Request(PDF_URL, headers={"User-Agent": "Parca-Avcisi-catalog-ingest/1.1"})
    with urllib.request.urlopen(req, timeout=180) as response, open(PDF_PATH, "wb") as out:
        while True:
...
def extract_records():
    ...
    if len(reader.pages) < START_PAGE:
        raise RuntimeError(f"Unexpected catalog page count: {len(reader.pages)}; start page is {START_PAGE}")
    ...
    for part in sorted(applications):
        ...
        records.append({"sourceId": SOURCE_ID, "brand": BRAND, "partNumber": part, "partName": f"{BRAND} {part}", "category": category(part), "oemNumbers": [], "applications": apps, "sourceUrl": SOURCE_URL, "sourceQuality": 0.99, "rawHash": hashlib.sha256(raw.encode("utf-8")).hexdigest()})
...
def rpc(batch):
    ...
    req = urllib.request.Request(SUPABASE_URL + "/rest/v1/rpc/upsert_catalog_batch", data=envelope, method="POST", headers={"apikey": SUPABASE_KEY, "Authorization": f"Bearer {SUPABASE_KEY}", "Content-Type": "application/json"})
...
if __name__ == "__main__":
    if not SOURCE_ID:
        raise RuntimeError("MANN_SOURCE_ID is required")
    download_pdf()
    upload(extract_records())
