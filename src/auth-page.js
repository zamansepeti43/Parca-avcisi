...
if (!supabaseConfigured) {
  ...
  // Kayıt sayfası tamamen bağımsızdır: mevcut oturum, SIGNED_IN/INITIAL_SESSION
  // olayları veya başka bir sekmedeki auth değişimi bu sayfayı kapatamaz.
  renderSignup();
} else {
  getCurrentUser().then((user) => { if (user) window.location.href = homeUrl; }).catch(() => {});
  onAuthStateChange((event) => {
    if (event === 'SIGNED_IN') window.location.href = homeUrl;
  });
