...
setProperty('og:image', `${SITE_URL}/app-logo.png`);
...
function updateListingItemList() {
  ...
  const cards = [...document.querySelectorAll('#listingGrid .listing-card')];
  if (!cards.length) return;
  addJsonLd('listing-item-list', {
    '@context':'https://schema.org', '@type':'ItemList', name:'Parça Avcısı oto parça ilanları',
    itemListElement: cards.slice(0, 24).map((card, index) => ({
      '@type':'ListItem', position:index + 1,
      name:card.querySelector('h3')?.textContent?.trim() || 'Oto parçası'
    }))
  });
}
if (isListings) {
  const observer = new MutationObserver(() => updateListingItemList());
  observer.observe(document.body, { childList:true, subtree:true });
  window.setTimeout(updateListingItemList, 1500);
}
