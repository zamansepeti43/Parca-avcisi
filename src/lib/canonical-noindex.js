...
const publicPage=path==='/'||path==='/ilanlar'||validCategory||validVehicle||legalPaths.has(path);
const hasQuery=[...params.keys()].length>0;
const shouldNoindex=accountPaths.has(path)||!publicPage||hasQuery||(p[0]==='arac'&&!validVehicle);
const canonical=validVehicle?`${SITE_URL}/arac/${p[1]}/${p[2]}`:validCategory?`${SITE_URL}/parcalar/${p[1]}`:legalPaths.has(path)?`${SITE_URL}${path}`:path==='/ilanlar'?`${SITE_URL}/ilanlar`:`${SITE_URL}${path==='/'?'':path}`;
