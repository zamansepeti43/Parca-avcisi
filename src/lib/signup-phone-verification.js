...
async function openEmailVerification(email, phone) {
  ...
  const modal = document.querySelector('#signupEmailVerifyModal'), form = document.querySelector('#signupEmailVerifyForm'), status = form.querySelector('[data-signup-email-status]'), resend = form.querySelector('[data-resend-email-code]');
  ...
  form.addEventListener('submit', async e => { e.preventDefault(); const button = form.querySelector('button:not([data-resend-email-code])'); button.disabled = true; try { const token = form.elements.emailOtp.value.trim(); if (!/^\d{6}$/.test(token)) throw new Error('6 haneli e-posta kodunu gir.'); status.textContent = 'E-posta kodu doğrulanıyor…'; await verifyEmailOtp(email, token); clearPendingEmail(); clearEmailResendTimer(); modal.remove(); const user = await getCurrentUser().catch(() => null); if (user) await openVerification(phone); else { reopenSignupParentModal(); window.__showToast?.('E-posta doğrulandı. Şimdi giriş yap.'); } } catch (error) { status.textContent = errorText(error); button.disabled = false; } });
  resend.addEventListener('click', async () => { if (resend.disabled) return; resend.disabled = true; status.textContent = 'Yeni e-posta kodu gönderiliyor…'; try { await resendEmailOtp(email); status.textContent = 'Yeni 6 haneli e-posta kodu gönderildi.'; startEmailResendCooldown(resend); } catch (error) { status.textContent = errorText(error); const m = String(error?.message || '').match(/(\d+)\s*(?:seconds?|saniye)/i); if (m) startEmailResendCooldown(resend, Math.max(1000, Number(m[1]) * 1000)); else resend.disabled = false; } });
  return true;
}

function phoneModalHtml(phone) { return '<div id="signupPhoneVerifyModal" class="app-modal show" aria-hidden="false"><div class="modal-card" role="dialog" aria-modal="true"><span class="eyebrow">KAYIT DOĞRULAMA</span><h2>Telefonunu doğrula</h2><p>Telefonuna gönderilen 6 haneli SMS kodunu gir.</p><form id="signupPhoneVerifyForm" class="stack-form"><input name="otp" inputmode="numeric" autocomplete="one-time-code" maxlength="6" pattern="[0-9]{6}" required placeholder="6 haneli SMS kodu"><button>Telefonu Doğrula</button><button type="button" data-resend-signup-sms class="secondary">SMS kodunu tekrar gönder</button><small data-signup-phone-status role="status"></small></form></div></div>'; }
async function openVerification(phone, send = true) {
  document.querySelector('#signupPhoneVerifyModal')?.remove(); const pending = getPending(); if (pending?.startedAt && Date.now() - Number(pending.startedAt) >= PHONE_OTP_UI_TTL_MS) { clearPending(); return false; }
  document.body.insertAdjacentHTML('beforeend', phoneModalHtml(phone)); const modal = document.querySelector('#signupPhoneVerifyModal'), form = document.querySelector('#signupPhoneVerifyForm'), status = form.querySelector('[data-signup-phone-status]');
