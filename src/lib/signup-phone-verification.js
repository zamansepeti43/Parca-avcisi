...
function setPendingEmail(email, phone) { sessionStorage.setItem(PENDING_EMAIL_KEY, JSON.stringify({ email: String(email || '').trim().toLowerCase(), phone, pageInstanceId: PAGE_INSTANCE_ID, createdAt: Date.now() })); }
function getPendingEmail() {
  try {
    const value = JSON.parse(sessionStorage.getItem(PENDING_EMAIL_KEY) || 'null');
    if (!value?.email || value.pageInstanceId !== PAGE_INSTANCE_ID) {
...
function errorText(error) {
  ...
  if (/cannot set properties of null|innerhtml/i.test(message)) return 'Kayıt güvenlik kontrolü hazırlanamadı. Lütfen tekrar dene.';
...
function ensureCaptchaContainer(form) {
  if (!(form instanceof HTMLFormElement)) return null;
  let container = form.querySelector('[data-turnstile-signup]') || form.querySelector('[data-turnstile]');
  if (container) return container;
  container = document.createElement('div');
  container.dataset.turnstileSignup = 'true';
...
async function getCaptchaToken(container, form) {
  const target = container || ensureCaptchaContainer(form);
  if (!target) throw new Error('Kayıt güvenlik alanı hazırlanamadı.');
  ...
  return new Promise((resolve, reject) => {
    ...
    target.dataset.widgetId = String(widgetId);
...
function startEmailResendCooldown(button, duration = EMAIL_RESEND_COOLDOWN_MS) {
  ...
  tick();
  emailResendTimer = setInterval(tick, 1000);
}

function emailModalHtml(email) {
  return '<div id="signupEmailVerifyModal" class="app-modal show" aria-hidden="false"><div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="signupEmailVerifyTitle" style="position:relative"><button type="button" data-close-email-verification aria-label="Geri dön" style="position:absolute;right:14px;top:12px;width:38px;height:38px;border:0;border-radius:50%;background:#f1f1f1;color:#171717;font-size:25px;cursor:pointer">×</button><span class="eyebrow">KAYIT DOĞRULAMA</span><h2 id="signupEmailVerifyTitle">E-postanı doğrula</h2><p><strong>' + email + '</strong> adresine gönderdiğimiz doğrulama kodunu gir.</p><form id="signupEmailVerifyForm" class="stack-form"><input name="emailOtp" inputmode="numeric" autocomplete="one-time-code" maxlength="8" minlength="6" pattern="[0-9]{6,8}" required placeholder="6 veya 8 haneli e-posta kodu"><button>E-postayı Doğrula</button><button type="button" data-resend-email-code class="secondary">Kodu tekrar gönder</button><small data-signup-email-status role="status"></small></form></div></div>';
}
...
async function openEmailVerification(email, phone) {
  ...
  form.addEventListener('submit', async e => {
    ...
    try {
      ...
      if (!new RegExp(`^\\d{${EMAIL_OTP_MIN_LENGTH},${EMAIL_OTP_MAX_LENGTH}}$`).test(token)) throw new Error('E-posta doğrulama kodunu 6-8 hane olarak gir.');
      ...
      if (user) await openVerification(phone); else { reopenSignupParentModal(); window.__showToast?.('E-posta doğrulandı. Şimdi telefon doğrulamasına geçebilirsin.'); }
    } catch (error) {
  ...
  resend.addEventListener('click', async () => {
    ...
    try {
      ...
      startEmailResendCooldown(resend);
    } catch (error) {
      status.textContent = errorText(error);
      const m = String(error?.message || '').match(/(\d+)\s*(?:seconds?|saniye)/i);
      if (m) startEmailResendCooldown(resend, Math.max(1000, Number(m[1]) * 1000)); else resend.disabled = false;
    }
  });
  return true;
}
...
function phoneModalHtml(phone) {
  return '<div id="signupPhoneVerifyModal" class="app-modal show" aria-hidden="false"><div class="modal-card" role="dialog" aria-modal="true" aria-labelledby="signupPhoneVerifyTitle" style="position:relative"><button type="button" data-close-phone-verification aria-label="Geri dön" style="position:absolute;right:14px;top:12px;width:38px;height:38px;border:0;border-radius:50%;background:#f1f1f1;color:#171717;font-size:25px;cursor:pointer">×</button><span class="eyebrow">KAYIT DOĞRULAMA</span><h2 id="signupPhoneVerifyTitle">Telefonunu doğrula</h2><p>Telefonuna gönderilen 6 haneli SMS kodunu gir.</p><form id="signupPhoneVerifyForm" class="stack-form"><input name="otp" inputmode="numeric" autocomplete="one-time-code" maxlength="6" pattern="[0-9]{6}" required placeholder="6 haneli SMS kodu"><button>Telefonu Doğrula</button><button type="button" data-resend-signup-sms class="secondary">SMS kodunu tekrar gönder</button><small data-signup-phone-status role="status"></small></form></div></div>';
}

async function openVerification(phone, send = true) {
  document.querySelector('#signupPhoneVerifyModal')?.remove();
  const pending = getPending();
  if (pending?.startedAt && Date.now() - Number(pending.startedAt) >= PHONE_OTP_UI_TTL_MS) { clearPending(); return false; }
  ...
  modal.addEventListener('click', e => { if (e.target === modal) close(); });
  try {
    if (send) { status.textContent = 'SMS gönderiliyor…'; await startPhoneVerification(phone); status.textContent = 'SMS kodu gönderildi. Kod 5 dakika geçerlidir.'; }
    else status.textContent = 'Aktif SMS doğrulama kodunu gir.';
  } catch (error) { status.textContent = errorText(error); }
  form.addEventListener('submit', async e => {
    e.preventDefault();
    const b = form.querySelector('button:not([data-resend-signup-sms])');
    b.disabled = true;
    ...
    } catch (error) { status.textContent = errorText(error); b.disabled = false; }
  });
  form.querySelector('[data-resend-signup-sms]').addEventListener('click', async () => {
    const b = form.querySelector('[data-resend-signup-sms]');
    b.disabled = true;
    status.textContent = 'SMS tekrar gönderiliyor…';
    try {
      await startPhoneVerification(phone);
      const p = getPending();
      if (p) { p.startedAt = Date.now(); sessionStorage.setItem(PENDING_KEY, JSON.stringify(p)); }
...
async function handleSignup(event) {
  ...
  if (!email) return window.__showToast?.('E-posta adresini gir.');
  ...
  try {
    const captcha = ensureCaptchaContainer(form);
    const captchaToken = await getCaptchaToken(captcha, form);
    ...
    if (result?.session) await openVerification(phone); else await openEmailVerification(email, phone);
  } catch (error) {
...
document.addEventListener('click', event => {
  const trigger = event.target.closest('#signupBtn, [data-open-signup]');
  if (!trigger) return;
  setTimeout(async () => {
    const pending = getPending();
    const user = await getCurrentUser().catch(() => null);
    if (pending && user && !user.phone_confirmed_at && pending.email === String(user.email || '').toLowerCase()) {
      if ((Date.now() - Number(pending.startedAt || 0)) < PHONE_OTP_UI_TTL_MS) {
...
async function resumePendingVerification(user) {
  ...
  if ((Date.now() - Number(pending.startedAt || 0)) >= PHONE_OTP_UI_TTL_MS) { clearPending(); return; }
  try { await openVerification(normalizePhone(pending.phone), false); } catch {}
}
