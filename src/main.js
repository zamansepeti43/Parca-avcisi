...
function accountRouteEmptyHtml(pane) {
  const states = {
    ilanlarim: ['Henüz ilan vermedin', 'Satmak istediğin parçayı ekle; ilanını fotoğraf, fiyat ve araç bilgileriyle yayınla.', '/ilan-ver', '＋ İlan Ver'],
    taleplerim: ['Henüz parça talebin yok', 'Bulamadığın parçayı tarif et; uygun satıcılar seni bulsun.', '/#parca-talebi', '＋ Parça Talebi Oluştur'],
    mesajlarim: ['Henüz mesajın yok', 'Bir ilan veya parça talebi üzerinden iletişim kurduğunda konuşmaların burada görünür.', '/ilanlar', 'Parça Bul →'],
    favorilerim: ['Henüz favorin yok', 'Beğendiğin ilanları kalp simgesiyle kaydet; daha sonra buradan hızlıca ulaş.', '/ilanlar', 'İlanlara Göz At →'],
    'kayitli-aramalar': ['Henüz kayıtlı araman yok', 'Sık aradığın parçaları kaydet; yeni uygun ilanları daha kolay takip et.', '/ilanlar', 'İlan Ara →'],
    bildirimler: ['Şimdilik bildirimin yok', 'Yeni mesaj, talep cevabı ve ilan durumları oluştuğunda bildirimlerin burada görünecek.', '/ayarlar', 'Ayarları Aç →'],
    musterilerim: ['Henüz müşterin yok', 'Bir parça talebine “Bende Var” ile cevap verdiğinde alıcılar burada görünür.', '/taleplerim', 'Taleplerime Git →'],
    'hesap-bilgileri': ['Hesap bilgilerin hazır', 'Profil bilgilerini güncellemek için Profilim bölümünü kullanabilirsin.', '/profilim', 'Profilimi Aç →'],
    ayarlar: ['Ayarların burada', 'Hesabınla ilgili tercihleri bu bölümden yönetebilirsin.', '/profilim', 'Profilimi Aç →'],
    'yardim-destek': ['Yardım & Destek', 'Bir sorun yaşarsan buradan destek ve kullanım bilgilerine ulaşabilirsin.', '/#yardim', 'Yardım Bilgileri →'],
  };
  const state = states[pane] || ['Profilin', 'Hesap bilgilerini burada yönetebilirsin.', '/profilim', 'Profilimi Aç →'];
  return '<div class="pane-empty account-route-fallback"><strong>' + state[0] + '</strong><span>' + state[1] + '</span><a class="empty-action" href="' + state[2] + '">' + state[3] + '</a></div>';
}
...
async function bootAccountRoute(route = activePath) {
  ...
  if (!pane || typeof window.__openAccountCenter !== 'function') { mount.innerHTML = accountRouteEmptyHtml(pane); return; }
  await window.__openAccountCenter(pane);
  let rendered = document.querySelector('#modalContent')?.innerHTML || '';
  if (!rendered) {
    await new Promise((resolve) => window.setTimeout(resolve, 180));
    if (typeof window.__openAccountCenter === 'function') await window.__openAccountCenter(pane);
    rendered = document.querySelector('#modalContent')?.innerHTML || '';
  }
  mount.innerHTML = rendered || accountRouteEmptyHtml(pane);
