...
async function bootAccountRoute() {
  ...
  mount.innerHTML = document.querySelector('#modalContent')?.innerHTML || '<div style="padding:40px">İçerik yüklenemedi.</div>';
  ...
  if (menu && !menu.querySelector('[data-pane="araclarim"]')) {
    const link = document.createElement('a'); link.className = 'account-menu-link'; link.dataset.pane = 'araclarim'; link.href = '/araclarim'; link.innerHTML = '<span aria-hidden="true">🚗</span><strong>Araçlarım</strong>';
    const profile = menu.querySelector('[data-pane="profilim"]'); if (profile?.parentNode) profile.parentNode.insertBefore(link, profile.nextSibling); else menu.prepend(link);
  }
